...
namespace sentinel::nexus_client
{
    ...
    bool NexusUplink::start(const NexusConfig &config)
    {
        ...
        heartbeat_thread_ = std::jthread([this](std::stop_token st)
                                         { heartbeat_worker(st); });
        defense_thread_ = std::jthread([this](std::stop_token st)
                                       { collective_defense_worker(st); });
        ota_thread_ = std::jthread([this](std::stop_token st)
                                   { ota_poll_worker(st); });
    ...
    {
        if (!connected_.load() || assigned_node_id_.empty())
            return;

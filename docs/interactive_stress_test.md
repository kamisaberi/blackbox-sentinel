An interactive **Attack Controller Console** is an amazing idea. 

Instead of running static scripts, this gives you a **live Interactive Terminal Control Panel** where you can turn individual attacks ON and OFF like light switches, change attack types, adjust intensity, or launch a full multi-vector assault while watching **Blackbox Sentinel** respond live on `http://localhost:8443`.

---

### File: Interactive Attack Controller (`scripts/attack_console.py`)

Save this file as **`scripts/attack_console.py`** in your project root:
...
1. Make the script executable:
   ```bash
   chmod +x scripts/attack_console.py
   ```

2. Open **Terminal 1** and start the Sentinel Daemon:
   ```bash
   sudo ./build/sentinel
   ```

3. Open **Terminal 2** and launch the Interactive Attack Console:
   ```bash
   sudo python3 scripts/attack_console.py
   ```

4. Open your browser to **`http://localhost:8443`**.

---

### What You Can Do Now in Real-Time:

* **Press `1`**: Starts/Stops a live Nmap port scan targeting Sentinel.
* **Press `2`**: Starts/Stops a live Modbus SCADA PLC write attack.
* **Press `3`**: Starts/Stops SSH brute forcing.
* **Press `5`**: Starts/Stops high-speed Syslog anomaly bursts.
* **Press `6`**: Launches **ALL 5 attacks simultaneously** to stress test Sentinel.
* **Press `7`**: Instantly stops all attacks and returns everything to idle!
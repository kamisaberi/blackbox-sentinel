...
sudo docker exec sim-attacker nmap -sS -p 22,80,443,502 127.0.0.1 || true

# 3. Launch simulated unauthorized Modbus PLC Write attack to port 502
echo -e "\n${RED}[Step 3] Launching unauthorized Modbus PLC Write attack to port 502...${NC}"
sudo docker exec sim-attacker sh -c "echo 'MALICIOUS_MODBUS_WRITE_COMMAND' | nc -w 1 127.0.0.1 502" || true

# 4. Check Sentinel REST API
echo -e "\n${GREEN}[Step 4] Querying Blackbox Sentinel REST API...${NC}"
